...
public class Storage(){
        ...
        private var root_tree: T.Tree<Text, Text> = T.Tree<Text, Text>(_key, _val, _parent, _childs, _level, _hm_capacity, Text.equal, Text.hash);
        // **Creating a new table**//
        private func add_table(
            table_key_name: Text): (Tree , Tree){
            let (instance, table) = T.add_child(table_key_name, table_key_name, root_tree);  
        ...
        private func create_table_and_column(
            ...
            switch(table){
                case(null){
                    ...
                    let (table_, column) = T.add_child(column_key_name, column_key_name, table);
                    return column; 
                };
                case(?table){
                    let (table_, column) = T.add_child(column_key_name, column_key_name, table);
        ...
        public func replace_table_value(
            ...
            switch(table){
                ...
                case(?table){
                    ...
                    switch(column){
                        case(null){
                            let (table_, column_) = T.add_child(column_key_name, column_key_name, table); //Create column
